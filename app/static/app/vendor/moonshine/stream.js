...
export class Stream {
    ...
    constructor(raw, updateInterval = DEFAULT_UPDATE_INTERVAL) {
        this.raw = raw;
    ...
    addAudio(audio, sampleRate, flags = TranscribeFlags.None) {
        ...
        if (sampleRate > 0)
            this.pending += audio.length / sampleRate;
    }
    /**
    ...
    transcribe(flags = TranscribeFlags.None) {
        ...
        const needed = Math.min(Math.max(this.updateInterval, this.lastPass), this.updateInterval * MAX_INTERVAL_FACTOR);
        if (!forced && this.pending < needed)
            return this.latest;
        this.pending = 0;
        let transcript;
