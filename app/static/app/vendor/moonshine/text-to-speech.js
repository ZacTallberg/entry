...
export class TextToSpeech {
    ...
    nextChunk() {
        ...
        if (chunk.status === TTS_NEED_TEXT || !chunk.audio)
            return undefined;
        return {
            audio: chunk.audio,
    ...
    async *stream(text) {
        ...
        let stopped = false;
        const feeding = text === undefined ? undefined : this.feedStream(text, () => stopped);
 ...
 */
export async function splitSayUtterances(text, options = {}) {
...
function splitSayUtterancesWith(module, text, language) {
    if (!module.ttsSplitUtterances) {
        throw new Error('TTS sentence splitting is unavailable in this build.');
    }
    const json = wrapErrors(() => module.ttsSplitUtterances(language ?? '', text));
