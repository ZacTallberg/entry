...
export class VoiceClone {
    ...
    progressCallbacks = [];
    stopCapture;
    constructor(module, ttsHandle, options = {}) {
        this.module = module;
        ...
        if (!module.extractSpeechClip) {
            throw new Error('This Moonshine WASM build does not support speech clip extraction.');
        }
    }
     ...
     */
    addAudio(pcm, sampleRate) {
        if (this.clip || pcm.length === 0)
            return;
    ...
    async fromMicrophone(options = {}) {
        ...
        node.onaudioprocess = (event) => {
            this.addAudio(new Float32Array(event.inputBuffer.getChannelData(0)), inputRate);
            if (this.clip) {
                finish(this.clip);
            }
                ...
                if (this.clip) {
                    ...
                    fail(new Error(`No speech detected in ${maxSeconds}s of recording. Try again somewhere quieter.`));
                }
            }
        };
        source.connect(node);
    ...
    search(options = {}) {
        ...
        const result = this.module.extractSpeechClip(recording, TARGET_SAMPLE_RATE, this.ttsHandle, this.clipSeconds, options.acceptAnything ? 0 : this.minimumSpeechSeconds);
 ...
 */
export async function extractSpeechClip(audio, sampleRate, options) {
    const module = options.module ?? (await loadMoonshineModule());
    if (!module.extractSpeechClip) {
        throw new Error('This Moonshine WASM build does not support speech clip extraction.');
    }
    const clipSeconds = options.clipSeconds ?? 4;
    const result = module.extractSpeechClip(audio, sampleRate, options.ttsHandle, clipSeconds, options.minimumSpeechSeconds ?? 2);
    if (result.isComplete && result.audio && result.audio.length > 0) {
        return result.audio;
    }
    ...
    if (resampled.length <= wanted)
        return resampled;
    const from = Math.min(Math.max(0, Math.round(result.startTime * TARGET_SAMPLE_RATE)), resampled.length - wanted);
