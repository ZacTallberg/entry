...
export class MicTranscriber {
    ...
    async start() {
        ...
        this.stream.start();
        const onChunk = (chunk) => {
            if (!this.running || this.muted || !this.stream)
                return;
 ...
 */
export function wrapProgress(callback) {
    if (!callback)
        return undefined;
